...
export function MultiSelect({
  ...
  const addCustomValue = useCallback(
    (value: string) => {
      ...
      setItems((prev) => {
        if (prev.has(trimmedValue)) return prev;
        return new Map(prev).set(trimmedValue, trimmedValue);
      });
    ...
    [allowCustomValues, selectedValues, onValuesChange]
  );
...
export function MultiSelectValue({
  ...
  const checkOverflow = useCallback(() => {
    ...
    for (let i = items.length - 1; i >= 0; i--) {
      const child = items[i];
      if (containerElement.scrollWidth <= containerElement.clientWidth || !child) {
        break;
      }
...
export function MultiSelectContent({
  ...
  const { allowCustomValues, addCustomValue, items, selectedValues } = useMultiSelectContext();
  const [searchValue, setSearchValue] = useState('');

  const hasExactMatch = searchValue.trim() && items.has(searchValue.trim());
  const shouldShowAddOption = allowCustomValues && searchValue.trim() && !hasExactMatch;

  // Get all custom values that are not explicitly defined as children
  const customValues = Array.from(items.keys()).filter((value) => {
    // Check if this value is already rendered as a child component
